...
#### genotype data ####
print("[LOG] read bgen by chr...")
bgs <- character()
for (chr in 1:22) { bgs <- append(bgs, 'training.bgen')}
idlist <- lapply(c(1:22), function(i) read.table('training.bgid')[,1])
snp_readBGEN(bgenfiles= bgs, 'training', idlist, read_as = "dosage", ncores=72)

obj.bigSNP <- snp_attach("training.rds")

# See how the file ddlooks like
str(obj.bigSNP, max.level = 2, strict.width = "cut")

pheno <- fread(pheno.file, data.table=FALSE)
sample <- fread('training.sample', data.table=FALSE)
sample <- sample[-1,]
sample_pheno <- merge(x=sample, y=pheno, by.x="ID_2", by.y="sample_id", all.x=TRUE)
...
colnames(sumstats) <- c("chr", "pos", "rsid", "a0", "a1", "ac", "freq", "info", "beta", "beta_se", "tstat", "var", "p", "pna", "isspa", "af_case", "af_ctrl", "n_case", "n_ctrl", "rs")
...
sumstats$n_eff <- 4 / ((1/sumstats$n_case ) + (1/sumstats$n_ctrl)) ### case/control

### match snp between GWAS & array
map <- setNames(obj.bigSNP$map[-2], c("chr", "rsid", "pos", "a0", "a1", "freq", "info"))
...
png(paste0(wd, phecode, '_ld_auc.png'))
print(plot1)
dev.off()
...
png(paste0(wd, phecode, '_lasso_auc.png'))
print(plot2)
dev.off()
...
w=merge(x=obj.bigSNP$map, y=info_snp, by.x='marker.ID', by.y='rsid', all.x=TRUE)
write.table(w[,c('marker.ID','a1','LDPred2weight','lassosum2weight')], paste0(wd, phecode,'_weight.txt'), sep="\t", col.names = c('snp','a1', phecode, paste0(phecode,'_lasso')), row.names = F, quote = F )
